...
/**
 * BIP32 Compatible Key generator.
 */
public class KeyGenerator {
    ...
    public ExtendedKey createExtendedKey(byte[] seed) throws KeyValidationException {
        try {
            ...
            if (m.compareTo(CURVE_PARAMS.getN()) >= 0) {
                throw new KeyValidationException(EXCEPTION_MESSAGE_UNLIKELY);
            }
            ECKeyPair keyPair = new ECKeyPair(l, true);
            return new ExtendedKey(keyPair, r, 0, 0, 0);
        } catch (NoSuchAlgorithmException | NoSuchProviderException | InvalidKeyException e) {
            throw new KeyValidationException(e);
        }
    }

    public ExtendedKey createExtendedKeyFromPassphrase(String passphrase, byte[] encryptedBytes) throws KeyValidationException {
        try {
            byte[] key = SCrypt.generate(passphrase.getBytes(StandardCharsets.UTF_8), SEED_PREFIX, 16384, 8, 8, 32);
    ...
    public ExtendedKey parseExtendedKey(String serialized) throws KeyValidationException {
        byte[] data = ByteUtils.fromBase58WithChecksum(serialized);
        if (data.length != 78) {
            throw new KeyValidationException("invalid extended key");
        }
        ...
        if (Arrays.areEqual(type, xprv) || Arrays.areEqual(type, tprv)) {
            ...
            throw new KeyValidationException("invalid magic number for an extended key");
        }
    ...
    public class ExtendedKey {
        ...
        public ECKeyPair getKey(int sequence) throws KeyValidationException {
            return generateKey(sequence).getMaster();
        }

        public ExtendedKey getChild(int sequence) throws KeyValidationException {
        ...
        public ExtendedKey generateKey(int sequence) throws KeyValidationException {
            try {
                if ((sequence & 0x80000000) != 0 && master.getPrivate() == null) {
                    throw new KeyValidationException(EXCEPTION_MESSAGE_NO_PRIVATE_KEY);
                }
        ...
        public byte[] encrypt(String passphrase, boolean production) throws KeyValidationException {
            try {
                byte[] key = SCrypt.generate(passphrase.getBytes(StandardCharsets.UTF_8), SEED_PREFIX, 16384, 8, 8, 32);
                SecretKeySpec keyspec = new SecretKeySpec(key, "AES");
                Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding", SECURITY_PROVIDER);
    ...
    public class ECKeyPair {
        ...
        public ECKeyPair(byte[] privateKeyBytes, boolean compressed) throws KeyValidationException {
            this(privateKeyBytes, compressed, false);
        }

        public ECKeyPair(byte[] pubOrPrivateKeyBytes, boolean compressed, boolean pubOnly) throws KeyValidationException {
            this.compressed = compressed;
        ...
        public BigInteger getWeb3PublicKey() {
            if (privateKey == null)
                return publicKey;
            ECPoint point = CURVE.getG().multiply(privateKey);
            byte[] encoded = point.getEncoded(false);
        ...
        public byte[] sign(byte[] hash) throws KeyValidationException {
            if (privateKey == null) {
                throw new KeyValidationException("Need private key to sign");
            }
